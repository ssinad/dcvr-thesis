...
std::pair<bool, Path> path_generation_orienteering(
    const Vertices &vertices,
    const Rewards &rewards,
    const Matrix &distances,
    const distance_t distance_limit_D,
...
std::unordered_map<int, PathWrapper> dcvr_fractional(
    const Vertices &vertices,
    const Node &root_node,
    const Matrix &distances,
    ...
    )
{
    ...
    do
    {
        std::clog << "Iteration #" << iter_cnt + 1 << std::endl;
        std::clog << "---------------------------" << std::endl;
        if (!solver.solve())
        {
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    p = b_path.second;
                    apx_called = true;
                    std::clog << "Path generated by orienteering" << std::endl;
                }
            }
        }
        ...
        assert(get_path_distance(p, distances) <= distance_limit_D + DISTANCE_EPSILON);
        ...
        if (apx_called && best_upper != 0)
        {
            ...
            std::clog << "Path approx. guarantee: " << apx << std::endl;
            if (apx < 1.0 / 3.0)
            {
            ...
            std::clog << "Lower bound on LP value: " << this_lb << std::endl;
            if (this_lb > best_lb)
                best_lb = this_lb;
        }
        
        std::clog << "Best upper bound: " << best_upper << std::endl;
        std::clog << "Best bound on LP value: " << best_lb << std::endl;
        std::clog << "Path reward: " << p_reward << std::endl;
        std::clog << "Reward residue: " << p_reward - 1 << std::endl;
    ...
    )
{
    ...
    std::clog << "Number of variables in MILP: " << path_cnt << std::endl;
    IloCplex solver(model);
    if (!solver.solve())
    {
        env.error() << "Failed to optimize LP" << endl;

...
// bool path_generation_heuristic_1(Path &generated_path, const Rewards &rewards, const Matrix &distances, const distance_t distance_limit_D, const Node &root_node)
...
{
    // std::cout << "Second heuristic called" << std::endl;
    Path best_path;
    ...
    do
    {
        ...
        for (Node i : vertices)
        {
            ...
            if (new_distance > distance_limit_D)
                continue;
            can_add_nodes = true;
            // cout << previous_node << " " << i << " : " << rewards[i] / distances[previous_node][i] << endl;
            // Pay attention to the datatypes when doing the division
            // TODO Should I take zero into consideration here?
...
std::pair<bool, Path> path_generation_orienteering(const Vertices &vertices, const Rewards &rewards, const Matrix &distances, const distance_t distance_limit_D, const Node &root_node)
{
    ...
    {
        std::cout << kv.second << ", ";
        // if (!isinf(kv.second) && isnan(kv.second) && kv.second > best_upper) best_upper = kv.second;
...
// Paths (Initialize to empty path vector) + fractional weights
...
//, std::unordered_map<int, Path> &paths, std::unordered_map<int, double> &values)
{
    // paths.clear();
    // values.clear();
    ...
    for (Node v : vertices)
    {
        ...
        if (distances[root_node][path_cnt] > distance_limit_D)
        {
            ...
            return paths; // TODO No solutions
        }
